...
function GetBooking() {
    ...
    console.log(data)
    $.post("Model/managerDeliveryModel.py", data, function (res) {
        res = JSON.parse(res)
        console.log(res)
        if (res != null) {
            $("#tb").show()
            $.each(res, function (k, v) {
                s = '<tr><td>' + v.bookdate + '</td>' +
                    ...
                    '<td>' + v.price + '</td>' +
                    '<td>' + v.shipdate + '</td>' +
                    '<td>' +  "<a href='#' id='"+v.bid+"' class='btn  btn-sm btn-outline-danger del'>Delivery</a> &nbsp;&nbsp;&nbsp;"
            ...
            $('.del').click(function (e) {
                ...
                window.location = "index_manager1.py?id=managerDelivery";
            })            
        } else {                     
        }      
    })
